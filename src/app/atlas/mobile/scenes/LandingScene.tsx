...
// ── Landing visual config keyed by state ──────────────────────────────────────
...
function OverviewScrolled({ onExplore, onBack }: { onExplore: () => void; onBack: () => void }) {
  ...
  return (
    <div style={{
      ...
      {/* Back tap zone at top gradient */}
      <div
        onClick={onBack}
        style={{
          height: 80, display: "flex", alignItems: "flex-end", paddingBottom: 10,
          paddingLeft: 28, cursor: "pointer",
        }}
      >
        <span style={{ fontFamily: T.mono, fontSize: 8.5, letterSpacing: "0.18em", color: T.gold, opacity: 0.32 }}>
          ‹ ATLAS
        </span>
      </div>
      <div style={{
...
interface LandingSceneProps {
  ...
  onOverviewExpand: () => void;   // system-awakened → system-overview
  onExplore: () => void;          // system-overview → case-studies-focus
  onBack: () => void;             // → atlas-landing
}
...
export default function LandingScene({
  ...
  return (
    <>
      {/* ── Landing SVG (always rendered as spatial context) ── */}
      <svg viewBox={`0 0 ${W} ${H}`} width={W} height={H}
        ...
        <path d={cs.orbitPath} fill="none" stroke={cs.color} strokeWidth={0.55}
          strokeDasharray="4.5 7" opacity={CS_ARC_OP[state]} style={{ transition: FADE }} />
        <path d={ex.orbitPath} fill="none" stroke={ex.color} strokeWidth={0.55}
          strokeDasharray="4.5 7" opacity={CTX_ARC_OP[state]} style={{ transition: FADE }} />
        ...
        {/* Case Studies — moves based on state */}
        <g style={{ opacity: csState.opacity, transition: FADE }}>
          <SystemNode
            ...
            awakened={isActive}
            dimmed={false}
            showLabel={state === "atlas-landing"}
          />
        </g>

        {/* Touch targets — landing only */}
        {state === "atlas-landing" && (
          <>
            <circle cx={95} cy={178} r={56} fill="transparent"
              onClick={onSelectCaseStudies} style={{ cursor: "pointer" }} />
            <circle cx={FW_POS.x} cy={FW_POS.y} r={56} fill="transparent"
              onClick={onSelectFrameworks} style={{ cursor: "pointer" }} />
          </>
        )}
      </svg>

      {/* ── Landing header (atlas-landing only) ── */}
      {state === "atlas-landing" && (
        <div style={{
      ...
      {/* Observatory entry (atlas-landing only) */}
      {state === "atlas-landing" && (
        <div style={{
      ...
      {state === "system-awakened" && (
        <div style={{
          ...
          <div onClick={onBack} style={{
            ...
            color: T.gold, opacity: 0.38, cursor: "pointer",
            pointerEvents: "auto", minHeight: 44, display: "flex", alignItems: "center",
          }}>
            ‹ ATLAS
          </div>
          <div style={{ fontFamily: T.mono, fontSize: 9, letterSpacing: "0.22em", color: T.caseStudies, opacity: 0.62 }}>
            CASE STUDIES
          </div>
        </div>
      )}

      {/* Overview Initial panel (system-awakened) */}
      {state === "system-awakened" && (
        <OverviewInitial onExplore={onOverviewExpand} />
      )}

      {/* Overview Scrolled full-screen (system-overview) */}
      {state === "system-overview" && (
        <OverviewScrolled onExplore={onExplore} onBack={onBack} />
      )}
    </>
  );
}
